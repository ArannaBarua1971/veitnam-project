...
function AllShortDeal() {
  ...
  const activeShortDeal = (id,index) => {
    console.log(id)
    axios
      .post(`${conf}/status_change_specific_short_deal/${id}`)
      .then((response) => {
  ...
  const deleteDeal = (id) => {
    axios
      .post(`${conf}/delete_short_deal/${id}`)
      .then((response) => {
        notify(`Delete deal `);
        getAllDeal();
      })
  ...
  return (
    <div>
      ...
      <div className="d-flex flex-wrap justify-content-between align-items-center mt-3">
        <ContentHeader className="text-dark">All Short Deal</ContentHeader>
        <Button
          className="py-2 my-3 bg-dark"
          onClick={() => navigate("/admin/short-deal")}
        >
          Add Short Deal
        </Button>
      </div>

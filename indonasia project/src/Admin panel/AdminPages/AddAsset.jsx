...
function AddAsset() {
  ...
  useEffect(() => {
    if (id) {
      setEdit(true);
      axios
        .get(`${conf}/get_specific_asset/${id}`)
        .then((response) => {
  ...
  const editContent = () => {
    ...
    axios
      .post(`${conf}/edit_specific_asset/${id}`, request)
      .then((response) => {
        notify(response.data.message);
        setTimeout(() => navigate("/admin/all-asset"), 2000);
      })
      .catch((error) => {
        console.log(error);
      });
    ...
    <div >
      ...
      <div className="shortDeal">
        ...
        <div className="d-flex flex-wrap justify-content-between align-items-center mt-3">
          <ContentHeader className="text-dark">Add Asset</ContentHeader>
          <Button
            className="py-2 my-3 bg-dark"
            onClick={() => navigate("/admin/all-Asset")}
          >
            All Asset
          </Button>
        </div>
        ...
        <div className="shortDealFrom d-flex flex-wrap add">
          ...
          <div className="popupEditor w-100">
            <hr className="my-2" />
            <ContentHeader>Popup for Asset </ContentHeader>
            <Input
              type="text"
